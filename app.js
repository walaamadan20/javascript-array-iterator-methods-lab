...
// Complete the exercise in the space below:
inventorNames = inventors.map((element) => {
...
console.log(inventors.map((element) => {
    return element.year
}))
...
//Comparing the first name to sort it
...
// // Complete the exercise in the space below:
firstLast = people.map((onePerson) => {
   const [lastName, firstName]  = onePerson.split(", ")
...
isAdultPresent = devs.some((adult) => {
    return (currentYear - adult.year) > 18
})
...
isEveryone19OrOlder = devs.every((adult) => {
    return (currentYear - adult.year) > 19
})
// Check your work:
